...
class PhotoAdapter(private var photoList: ArrayList<PhotoRow>) : RecyclerView.Adapter<DefaultViewHolder>() {
  ...
  override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): DefaultViewHolder {
    ...
    when (viewType) {
      RowType.PHOTO.ordinal -> inflatedView = layoutInflator?.inflate(R.layout.row_item, parent, false)
      RowType.HEADER.ordinal -> inflatedView = layoutInflator?.inflate(R.layout.header_item, parent, false)
    }
  ...
  class PhotoRowDiffCallback(private val newRows: List<PhotoRow>, private val oldRows: List<PhotoRow>) : DiffUtil.Callback() {
    ...
    override fun areContentsTheSame(oldItemPosition: Int, newItemPosition: Int): Boolean {
      val oldRow: PhotoRow = oldRows[oldItemPosition]
      val newRow: PhotoRow = newRows[newItemPosition]
